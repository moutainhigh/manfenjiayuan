 ...
 * 微信分享接口返回响应结果时调用这个类
 */
public class WXEntryActivity extends BaseActivity implements IWXAPIEventHandler, OAuthListener {
...
//                goToShowMsg((ShowMessageFromWX.Req) req);
    ...
    public void onResp(BaseResp resp) {
        ZLogger.d(String.format("openid = %s, errCode=%d, type=%d",
                resp.openId, resp.errCode, resp.getType()));
        int result = 0;
        switch (resp.errCode) {
            case BaseResp.ErrCode.ERR_OK:
                ...
                if (resp.getType() == ConstantsAPI.COMMAND_SENDAUTH) {
                    ...
                    try {
                        ...
                        ZLogger.d(String.format("code = %s, state=%s, url=%s",
                                respAuth.code, respAuth.state, respAuth.url));
                        getAccessToken(respAuth.code);
                    } catch (Exception e) {
                        e.printStackTrace();
