...
public class ShelveSyncManager {
    ...
    public void stepUploadData(final ShelveEntity shelveEntity) {
        ...
        if (L2CSyncStatus.SYNC_STATUS_FINISHED.compareTo(shelveEntity.getSyncStatus()) <= 0){
            uploadFailed(String.format("订单状态(%d)无效，不需要同步...", shelveEntity.getSyncStatus()));
            return;
        }

        if (!MfhLoginService.get().haveLogined()) {
            uploadFailed("会话已失效，暂停同步货架绑定纪录数据。");
            return;
        }
        ...
        uploadProcess(String.format("准备上传货架绑定纪录(%d/%s)", shelveEntity.getId(),
