...
public class HandOverBill implements java.io.Serializable{
    ...
    //按业务类型
    private List<HandoverAggItem> aggItems;
    private AggWrapper aggWrapper = new AggWrapper();


    //按支付类型
    private List<HandoverAccItem> accItems;
    ...
    public List<HandoverAccItem> getAccItems() {
        return accItems;
    }
    ...
    public List<HandoverAggItem> getAggItems() {
        return aggItems;
    }

    public void setAggItems(List<HandoverAggItem> aggItems) {
        this.aggItems = aggItems;
        if (this.aggWrapper == null){
            this.aggWrapper = new AggWrapper();
        }
        this.aggWrapper.initWithHandoverAggShift(aggItems);
    ...
    /**
     * 设置交接明细（按支付类型）*/
    public void setAccItems(List<HandoverAccItem> accItems) {
        this.accItems = accItems;
        if (this.accWrapper == null){
            this.accWrapper = new AccWrapper();
        }
        this.accWrapper.initWithHandoverAccItems(accItems);
        this.cash = this.accWrapper.getCashAmount();
