...
import com.mfh.litecashier.bean.CategoryInfo;
...
public class DataSyncManager {
    ...
    private static final int MAX_SYNC_PRODUCTS_PAGESIZE = 40;
    ...
    private void networkError() {
        ZLogger.df("DataSync--网络未连接，暂停同步POS数据。");
        bSyncInProgress = false;
        EventBus.getDefault().post(new DataSyncEvent(DataSyncEvent.EVENT_ID_SYNC_DATA_FINISHED));
    }

    private void sessionError() {
        ZLogger.df("DataSync--会话已失效，暂停同步POS数据。");
        bSyncInProgress = false;
        EventBus.getDefault().post(new DataSyncEvent(DataSyncEvent.EVENT_ID_SYNC_DATA_FINISHED));
    }
    ...
    private String getPosLastUpdateCursor() {
        ...
        ZLogger.df(String.format("DataSync--上次商品更新时间(%s)。", lastCursor));
...
//                    ZLogger.df(String.format("DataSync--上次商品更新时间大于当前时间，使用当前时间(%s)。", lastCursor));
//                }
//            } catch (ParseException e) {
////            e.printStackTrace();
//                ZLogger.ef(String.format("DataSync--获取POS商品库同步时间失败: %s", e.toString()));
//            }
//        }
    ...
    private void startSyncProducts() {
        ...
        if (SharedPreferencesHelper.getSyncProductsMode() == 0) {
            ...
            ZLogger.df("DataSync--同步商品库：全量更新，重置游标，删除旧数据");
        }
        //从第一页开始请求，每页最多50条记录
...
//        params.put("JSESSIONID", MfhLoginService.get().getCurrentSessionId());
        ...
        ZLogger.df(String.format("DataSync--同步商品开始(%d/%d/%s)", pageInfo.getPageNo(), pageInfo.getTotalPage(), lastCursor));
...
//        return null;
        ...
        protected void onPostExecute(Long aLong) {
            ...
            if (!isInterrupted && pageInfo.hasNextPage()) {
                ...
                ZLogger.df("DataSync--同步商品库结束" + SharedPreferencesHelper.getSyncProductsCursor());
...
//                nextStep();
        ...
        private void saveQueryResult(RspQueryResult<PosGoods> rs, PageInfo pageInfo) {//此处在主线程中执行。
            try {
                ...
                ZLogger.df(String.format("DataSync--同步 %d/%d 个商品（%s）", rs.getReturnNum(),
                        rs.getTotalNum(), SharedPreferencesHelper.getSyncProductsCursor()));
            } catch (Throwable ex) {
//            throw new RuntimeException(ex);
                ZLogger.e(String.format("DataSync--保存商品库失败: %s", ex.toString()));
                isInterrupted = true;
            }
        }
    }
...
//                        {"code":"0","msg":"查询成功!","version":"1","data":{"val":"701"}}
                        try {
                            ...
                            ZLogger.df(String.format("DataSync--指定网点可同步sku总数:%d", skuNum));
                            ...
                            if (posNum != skuNum) {
                                ZLogger.df(String.format("DataSync--pos本地商品数目(%d)和后台商品数目(%d)不一致,下一次需要全量同步商品库", posNum, skuNum));
...
//                                SharedPreferencesHelper.
                                ...
                                ZLogger.df(String.format("DataSync--pos本地商品数目(%d)和后台商品数目(%d)一致", posNum, skuNum));
                            }
                            nextStep();
                        } catch (Exception e) {
                            ZLogger.ef(String.format("DataSync--查询指定网点可同步sku总数:%s", e.toString()));
                            nextStep();
                        }
                    }
                    ...
                    protected void processFailure(Throwable t, String errMsg) {
                        super.processFailure(t, errMsg);
                        ZLogger.df("DataSync--查询指定网点可同步sku总数失败：" + errMsg);
...
//        params.put("JSESSIONID", MfhLoginService.get().getCurrentSessionId());
        ...
        ZLogger.df(String.format("DataSync--同步规格码表开始(%d/%d)", pageInfo.getPageNo(), pageInfo.getTotalPage()));
...
//        return null;
        ...
        protected void onPostExecute(Long aLong) {
            ...
            if (pageInfo.hasNextPage()) {
                ...
                ZLogger.df("DataSync--同步规格码表品结束:" + SharedPreferencesHelper.getSyncProductSkuCursor());
        ...
        private void saveQueryResult(RspQueryResult<ProductSkuBarcode> rs, PageInfo pageInfo) {//此处在主线程中执行。
            try {
                ...
                ZLogger.df(String.format("DataSync--同步 %d/%d 个箱规", retSize, rs.getTotalNum()));

            } catch (Throwable ex) {
//            throw new RuntimeException(ex);
                ZLogger.ef(String.format("DataSync--同步码表失败: %s", ex.toString()));
            }
        }
    }
    ...
    public void downloadWorkerBeanInfoOfCompany(PageInfo pageInfo) {
        ...
        NetCallBack.QueryRsCallBack queryRsCallBack = new NetCallBack.QueryRsCallBack<>(new NetProcessor.QueryRsProcessor<CompanyHuman>(pageInfo) {
            ...
            protected void processFailure(Throwable t, String errMsg) {
                super.processFailure(t, errMsg);
                ZLogger.df("DataSync--同步部门员工数据失败:" + errMsg);
                nextStep();
            }
        }, CompanyHuman.class, CashierApp.getAppContext());


        ZLogger.df(String.format("DataSync--同步部门员工数据开始(%d/%d)", pageInfo.getPageNo(), pageInfo.getTotalPage()));
        NetFactory.getHttp().post(CashierApi.URL_COMPANYHUMAN_LIST_WORKERBEANINFO_OFCOMPANY, params, queryRsCallBack);
    }
...
//        return null;
        ...
        protected void onPostExecute(Long aLong) {
            ...
            if (pageInfo.hasNextPage()) {
                ...
                ZLogger.df("DataSync--同步部门员工数据结束");
                nextStep();
            }
        }
        ...
        private void saveQueryResult(RspQueryResult<CompanyHuman> rs, PageInfo pageInfo) {//此处在主线程中执行。
            try {
                ...
                //保存下来
                int retSize = rs.getReturnNum();
                ZLogger.df(String.format("DataSync--同步 %d/%d 个部门员工数据", retSize, rs.getTotalNum()));
...
//            throw new RuntimeException(ex);
                ZLogger.e(String.format("DataSync--同步部门员工数据失败: %s", ex.toString()));
            }
        }
    }
    ...
    public void downloadCompanyHuman(PageInfo pageInfo) {
        ...
        ZLogger.df(String.format("DataSync--同步账号数据开始(%d/%d)", pageInfo.getPageNo(), pageInfo.getTotalPage()));
        NetFactory.getHttp().post(CashierApi.URL_COMPANYHUMAN_FIND_COMPUSERPWDINFO, params, queryRsCallBack);
    }
...
//        return null;
        ...
        private void saveQueryResult(RspQueryResult<CompanyHuman> rs, PageInfo pageInfo) {//此处在主线程中执行。
            try {
                ...
                //保存下来
                int retSize = rs.getReturnNum();
                ZLogger.df(String.format("DataSync--保存 %d/%d 个账号数据", retSize, rs.getTotalNum()));
                for (EntityWrapper<CompanyHuman> wrapper : rs.getRowDatas()) {
                    CompanyHumanService.get().saveOrUpdate(wrapper.getBean());
                }
...
//            throw new RuntimeException(ex);
                ZLogger.ef(String.format("DataSync--保存账号数据失败: %s", ex.toString()));
            }
        }
    }
    ...
    private void downloadBackendCategoryInfo() {
        if (!SharedPreferencesHelper.getBoolean(SharedPreferencesHelper.PK_SYNC_BACKEND_CATEGORYINFO_ENABLED, true)) {
            ZLogger.df("DataSync--使用后台类目缓存数据，暂不加载新数据");
            nextStep();
            return;
        }
    ...
    private NetCallBack.NetTaskCallBack backendCategoryInfoRspCallback = new NetCallBack.NetTaskCallBack<CategoryInfo,
            ...
            new NetProcessor.Processor<CategoryInfo>() {
                ...
                protected void processFailure(Throwable t, String errMsg) {
                    super.processFailure(t, errMsg);
                    ZLogger.df("DataSync--加载后台类目树失败, " + errMsg);
                    nextStep();
                }
...
//                            java.lang.ClassCastException: com.mfh.comn.net.data.RspListBean cannot be cast to com.mfh.comn.net.data.RspValue
    ...
    private void saveBackendCategoryInfoCache(List<CategoryOption> options) {
        ZLogger.df(String.format("DataSync--保存POS %d个后台类目",
    ...
    private void downloadBackendFreshCategoryInfo() {
        if (!SharedPreferencesHelper.getBoolean(SharedPreferencesHelper.PK_SYNC_BACKEND_CATEGORYINFO_FRESH_ENABLED, true)) {
            ZLogger.df("DataSync--使用后台生鲜类目缓存数据，暂不加载新数据");
            nextStep();
            //通知刷新数据
        ...
        ZLogger.df("DataSync--下载后台生鲜类目树开始");
    ...
    private NetCallBack.NetTaskCallBack freshBackendCategoryInfoRspCallback = new NetCallBack.NetTaskCallBack<CategoryInfo,
            ...
            new NetProcessor.Processor<CategoryInfo>() {
                ...
                protected void processFailure(Throwable t, String errMsg) {
                    super.processFailure(t, errMsg);
                    ZLogger.df("DataSync--加载后台生鲜类目树失败, " + errMsg);
                    //通知刷新数据
                    EventBus.getDefault().post(new DataSyncEvent(DataSyncEvent.EVENT_ID_REFRESH_BACKEND_CATEGORYINFO_FRESH));
...
//                            java.lang.ClassCastException: com.mfh.comn.net.data.RspListBean cannot be cast to com.mfh.comn.net.data.RspValue
    ...
    private void saveBackendFreshCategoryInfoCache(List<CategoryOption> options) {
        ...
        JSONArray cacheArrays = new JSONArray();
        if (options != null && options.size() > 0) {
            ZLogger.df(String.format("DataSync--保存 %d个后台生鲜类目", options.size()));
            for (CategoryOption option : options) {
                cacheArrays.add(option);
            }
    ...
    private void downloadPublicFrontCategory() {
        if (!SharedPreferencesHelper.getBoolean(SharedPreferencesHelper.PK_SYNC_PUBLIC_FRONTCATEGORY_ENABLED, true)) {
            ZLogger.df("DataSync--使用前台公共类目缓存数据，暂不加载新数据");
            //更新UI
//            EventBus.getDefault().post(new CashierAffairEvent(CashierAffairEvent.EVENT_ID_CLOUD_CATEGORY_UPDATED));
        ...
        ZLogger.df("DataSync--同步前台公共类目开始");
        CateApiImpl.listCategory(CateApi.DOMAIN_TYPE_PROD, CateApi.POS, CateApi.CATE_POSITION_FRONT,
    ...
    private NetCallBack.NetTaskCallBack publicFrontCategoryRespCallback = new NetCallBack.NetTaskCallBack<CategoryInfo,
            ...
            new NetProcessor.Processor<CategoryInfo>() {
                ...
                protected void processFailure(Throwable t, String errMsg) {
                    super.processFailure(t, errMsg);
                    ZLogger.df("DataSync--加载前台公共类目树失败, " + errMsg);
                    nextStep();
                }
                ...
                public void processResult(IResponseData rspData) {
                    ...
                    ZLogger.df("DataSync--加载前台公共类目树成功, ");
//                            java.lang.ClassCastException: com.mfh.comn.net.data.RspListBean cannot be cast to com.mfh.comn.net.data.RspValue
                    RspBean<CategoryInfo> retValue = (RspBean<CategoryInfo>) rspData;
                    CategoryInfo categoryInfo = retValue.getValue();
...
//            EventBus.getDefault().post(new CashierAffairEvent(CashierAffairEvent.EVENT_ID_CLOUD_CATEGORY_UPDATED));
        ...
        ZLogger.df(String.format("DataSync--同步前台公共二级类目(%s)开始", option.getValue()));
...
//                        1:12345卡芯片号不存在，请重新输入!
                        ZLogger.df("DataSync--加载POS前台公共二级类目 失败, " + errMsg);
                        nextStep();
                    }
                }
                , PosCategory.class
    ...
    private void savePublicFrontCategoryInfoCache(List<PosCategory> options) {
        ZLogger.df(String.format("DataSync--加载POS %d个前台公共二级类目",
                (options != null ? options.size() : 0)));
        //缓存数据
        JSONArray cacheArrays = new JSONArray();
        if (options != null && options.size() > 0) {
...
//                PosCategory category = PosCategory.generateCloud(option.getCode(), option.getValue(), option.);
        ...
        ACache.get(CashierApp.getAppContext(), ACacheHelper.CACHE_NAME).put(ACacheHelper.CK_PUBLIC_FRONT_CATEGORY, cacheArrays.toJSONString());
    ...
    private void downloadCustomFrontCategory() {
        if (!SharedPreferencesHelper.getBoolean(SharedPreferencesHelper.PK_SYNC_CUSTOM_FRONTCATEGORY_ENABLED, true)) {
            ZLogger.df("DataSync--使用前台自定义（私有）类目缓存数据，暂不加载新数据");
            //更新UI
//            EventBus.getDefault().post(new CashierAffairEvent(CashierAffairEvent.EVENT_ID_CLOUD_CATEGORY_UPDATED));
            nextStep();
            return;
        ...
        ZLogger.df("DataSync--同步前台自定义（私有）类目开始");
        CateApiImpl.listCategory(CateApi.DOMAIN_TYPE_PROD, CateApi.POS, CateApi.CATE_POSITION_FRONT,
    ...
    private NetCallBack.NetTaskCallBack customFrontCategoryRespCallback = new NetCallBack.NetTaskCallBack<CategoryInfo,
            ...
            new NetProcessor.Processor<CategoryInfo>() {
                ...
                protected void processFailure(Throwable t, String errMsg) {
                    super.processFailure(t, errMsg);
                    ZLogger.df("DataSync--加载前台自定义（私有）类目树失败, " + errMsg);
                    nextStep();
                }
...
//                            java.lang.ClassCastException: com.mfh.comn.net.data.RspListBean cannot be cast to com.mfh.comn.net.data.RspValue
    ...
    private void downloadCustomFrontCategory2(CategoryInfo categoryInfo) {
        ...
        List<CategoryOption> options = categoryInfo.getOptions();
        if (options == null || options.size() < 1) {
            ZLogger.df("DataSync--前台自定义（私有）类目为空");
            saveCustomFrontCategoryInfoCache(null);
            EventBus.getDefault().post(new DataSyncEvent(DataSyncEvent.EVENT_ID_REFRESH_FRONT_CATEGORYINFO));
            nextStep();
            return;
        ...
        CategoryOption option = options.get(0);
        ZLogger.df(String.format("DataSync--同步前台自定义（私有）二级类目(%s)开始", option.getValue()));
        NetCallBack.NetTaskCallBack queryRsCallBack = new NetCallBack.NetTaskCallBack<PosCategory,
                NetProcessor.Processor<PosCategory>>(
                new NetProcessor.Processor<PosCategory>() {
                    @Override
                    ...
                    protected void processFailure(Throwable t, String errMsg) {
                        super.processFailure(t, errMsg);
                        ZLogger.df("DataSync--加载前台自定义（私有）二级类目 失败, " + errMsg);
                        nextStep();
                    }
                }
                , PosCategory.class
    ...
    private void saveCustomFrontCategoryInfoCache(List<PosCategory> options) {
        ZLogger.df(String.format("DataSync--保存POS %d个前台自定义（私有）二级类目",
                (options != null ? options.size() : 0)));
        //缓存数据
        JSONArray cacheArrays = new JSONArray();
        if (options != null && options.size() > 0) {
    ...
    public static class DataSyncEvent {
        ...
        public static final int EVENT_ID_REFRESH_BACKEND_CATEGORYINFO       = 0X02;//刷新后台类目树
        public static final int EVENT_ID_REFRESH_BACKEND_CATEGORYINFO_FRESH = 0X03;//刷新后台生鲜类目树
