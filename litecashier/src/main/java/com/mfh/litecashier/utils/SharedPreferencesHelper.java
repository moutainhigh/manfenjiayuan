...
public class SharedPreferencesHelper {
    ...
    public static String getUploadOrderLastUpdate() {
        ...
        ZLogger.d(String.format("OrderSync--上次订单更新时间(%s)。", lastCursor));
...
//                    SharedPreferencesHelper.setPosOrderLastUpdate(d2);
                    ZLogger.d(String.format("OrderSync--上次订单更新时间大于当前时间，使用当前时间(%s)。", lastCursor));
                }
...
//            e.printStackTrace();
    ...
    public static void setLastHandoverDateTime(String lastUpdate) {
        set(PK_LAST_HANDOVER_DATETIME, lastUpdate);
    }

    public static void setLastHandoverDateTime(Date lastUpdate) {
        if (lastUpdate != null) {
            set(PK_LAST_HANDOVER_DATETIME, TimeCursor.InnerFormat.format(lastUpdate));
        }
    }
