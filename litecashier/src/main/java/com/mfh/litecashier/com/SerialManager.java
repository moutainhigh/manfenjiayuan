...
//    public static final String BAUDRATE_SCREEN = "2400";
    //秤(ACS-P215计价秤)
    public static final String PORT_SCALE = "/dev/ttyS0";
    public static final String BAUDRATE_SCALE = "9600";
    ...
    private static final String PREF_KEY_SCALE_PORT = "PREF_KEY_SCALE_PORT";
    private static final String PREF_KEY_SCALE_BAUDRATE = "PREF_KEY_SCALE_BAUDRATE";
    ...
    public static String getScalePort() {
        return SharedPreferencesManager.getText(
                PREF_NAME_SERIAL, PREF_KEY_SCALE_PORT, PORT_SCALE);
    }

    public static void setScalePort(String port){
        ZLogger.df(String.format("setScalePort(%s):%s", PREF_NAME_SERIAL, port));
        SharedPreferencesManager.set(PREF_NAME_SERIAL, PREF_KEY_SCALE_PORT, port);
    }

    public static String getScaleBaudrate() {
        return SharedPreferencesManager.getText(
                PREF_NAME_SERIAL, PREF_KEY_SCALE_BAUDRATE, BAUDRATE_SCALE);
    }

    public static void setScaleBaudrate(String baudrate){
        ZLogger.df(String.format("setScaleBaudrate(%s):%s", PREF_NAME_SERIAL, baudrate));
        SharedPreferencesManager.set(PREF_NAME_SERIAL, PREF_KEY_SCALE_BAUDRATE, baudrate);
    }
     ...
     * 打印POS订单流水
     * */
    public static void printPosOrder(PosOrderEntity orderEntity, boolean withCode128){
        if (orderEntity == null){
            return;
        }
...
//                EscCommand.ENABLE.OFF, EscCommand.ENABLE.OFF, EscCommand.ENABLE.OFF);
        ...
        /**打印 订购日期*/
        esc.addText(String.format("%s \n", TimeCursor.FORMAT_YYYYMMDDHHMMSS.format(orderEntity.getCreatedDate())));
        esc.addText("--------------------------------\n");//32个
...
//                makeTemp(esc, entity.getName(), String.format("%.2f", entity.getCostPrice() * entity.getBcount()));
                makeEnjoycityTemp(esc, String.format("%s/%s", entity.getBarcode(), entity.getName()), String.format("%.2f", entity.getCostPrice()), String.format("%.2f", entity.getBcount()), String.format("%.2f", entity.getCostPrice() * entity.getBcount()));
            }
        }
...
//        }
        ...
        makeHandoverTemp(esc, "1.线下收银", String.format("%.2f", aggWrapper.getPosQuantity()),
                String.format("%.2f", aggWrapper.getPosAmount()));
        makeHandoverTemp(esc, "2.线上订单", String.format("%.2f", aggWrapper.getScQuantity()),
                String.format("%.2f", aggWrapper.getScAmount()));
        makeHandoverTemp(esc, "3.衣物洗护", String.format("%.2f", aggWrapper.getLaundryQuantity()),
                String.format("%.2f", aggWrapper.getLaundryAmount()));
        makeHandoverTemp(esc, "4.皮具护理", String.format("%.2f", aggWrapper.getPijuQuantity()),
                String.format("%.2f", aggWrapper.getPijuAmount()));
        makeHandoverTemp(esc, "5.快递代收", String.format("%.2f", aggWrapper.getCourierQuantity()),
                String.format("%.2f", aggWrapper.getCourierAmount()));
        makeHandoverTemp(esc, "6.快递代揽", String.format("%.2f", aggWrapper.getExpressQuantity()),
                String.format("%.2f", aggWrapper.getExpressAmount()));
        makeHandoverTemp(esc, "7.转账充值", String.format("%.2f", aggWrapper.getRechargeQuantity()),
                String.format("%.2f", aggWrapper.getRechargeAmount()));

        esc.addPrintAndLineFeed();//进纸一行
        esc.addText("支付类型            数量    金额\n");

        AccWrapper accWrapper = handOverBill.getAccWrapper();
        makeHandoverTemp(esc, "1.现金收取", String.format("%.2f", accWrapper.getCashQuantity()),
                String.format("%.2f", accWrapper.getCashAmount()));
        makeHandoverTemp(esc, "2.支付宝支付", String.format("%.2f", accWrapper.getAlipayQuantity()),
                String.format("%.2f", accWrapper.getAlipayAmount()));
        makeHandoverTemp(esc, "3.微信支付", String.format("%.2f", accWrapper.getWxQuantity()),
                String.format("%.2f", accWrapper.getWxAmount()));
        makeHandoverTemp(esc, "4.会员账号支付", String.format("%.2f", accWrapper.getMemberAccountQuantity()),
                String.format("%.2f", accWrapper.getMemberAccountAmount()));
//        makeHandoverTemp(esc, "5.线上订单支付", String.format("%.2f", accWrapper.getScQuantity()),
// String.format("%.2f", accWrapper.getScAmount()));
        makeHandoverTemp(esc, "5.银行卡支付", String.format("%.2f", accWrapper.getBankcardQuantity()),
                String.format("%.2f", accWrapper.getBankcardAmount()));
...
//        DailysettleAggWrapper aggWrapper = handOverBill.getAggWrapper();
        makeHandoverTemp(esc, "1.线下收银", String.format("%.2f", dailysettleEntity.getAggPosOrderNum()),
                String.format("%.2f", dailysettleEntity.getAggPosAmount()));
        makeHandoverTemp(esc, "2.线上订单", String.format("%.2f", dailysettleEntity.getAggScOrderNum()),
                String.format("%.2f", dailysettleEntity.getAggScAmount()));
        makeHandoverTemp(esc, "3.衣物洗护", String.format("%.2f", dailysettleEntity.getAggLaundryOrderNum()),
                String.format("%.2f", dailysettleEntity.getAggLaundryAmount()));
        makeHandoverTemp(esc, "4.皮具护理", String.format("%.2f", dailysettleEntity.getAggPijuOrderNum()),
                String.format("%.2f", dailysettleEntity.getAggPijuAmount()));
        makeHandoverTemp(esc, "5.快递代收", String.format("%.2f", dailysettleEntity.getAggCourierOrderNum()),
                String.format("%.2f", dailysettleEntity.getAggCourierAmount()));
        makeHandoverTemp(esc, "6.快递代揽", String.format("%.2f", dailysettleEntity.getAggExpressOrderNum()),
                String.format("%.2f", dailysettleEntity.getAggExpressAmount()));
        makeHandoverTemp(esc, "7.转账充值", String.format("%.2f", dailysettleEntity.getAggRechargeOrderNum()),
                String.format("%.2f", dailysettleEntity.getAggRechargeAmount()));
...
//        AccWrapper accWrapper = handOverBill.getAccWrapper();
        makeHandoverTemp(esc, "1.现金收取", String.format("%.2f", dailysettleEntity.getAccCashOrderNum()),
                String.format("%.2f", dailysettleEntity.getAccCashAmount()));
        makeHandoverTemp(esc, "2.支付宝支付", String.format("%.2f", dailysettleEntity.getAccAlipayOrderNum()),
                String.format("%.2f", dailysettleEntity.getAccAlipayAmount()));
        makeHandoverTemp(esc, "3.微信支付", String.format("%.2f", dailysettleEntity.getAccWxOrderNum()),
                String.format("%.2f", dailysettleEntity.getAccWxAmount()));
        makeHandoverTemp(esc, "4.会员账号支付", String.format("%.2f", dailysettleEntity.getAccMemberOrderNum()),
                String.format("%.2f", dailysettleEntity.getAccMemberAccount()));
//        makeHandoverTemp(esc, "5.线上订单支付", String.format("%.2f", dailysettleEntity.getAccScOrderNum()),
//                String.format("%.2f", dailysettleEntity.getAccScAmount()));
        makeHandoverTemp(esc, "5.银行卡支付", String.format("%.2f", dailysettleEntity.getAccBankcardOrderNum()),
                String.format("%.2f", dailysettleEntity.getAccBankcardAmount()));
        ...
        esc.addText(String.format("毛利率:%.2f%%\n", 100 * grossMargin));
...
//        ZLogger.d("BarCode:" + cmdStr);
//        ZLogger.d("BarCode:" + cmdStr2);
//        ZLogger.d(String.format("printBarCode: %s, %s", content1, cmdStr4));
...
//        byte[] bytes = ArrayUtils.toPrimitive(Bytes);
     ...
     *  商品ID    品名   数量   单价   金额
     * */
    private static EscCommand makeTemp2(EscCommand rawEsc, String id, String name, String quantity, String price, String amount){
        EscCommand esc = rawEsc;
        if (esc == null){
            esc = new EscCommand();
        }
...
//        int maxLine = Math.max(1, len/7);
        int maxLine = Math.max(1, (name != null ? ((getLength(name) - 1) / 8 + 1) : 0));
...
//            ZLogger.d(String.format("subName2=%s nameTemp=%s", sub2, nameTemp));
//            line.append(formatLong(sub2, 8));
            line.append(sub2 + addBlank(Math.max(8 - getLength(sub2), 0)));
//
//            try {
