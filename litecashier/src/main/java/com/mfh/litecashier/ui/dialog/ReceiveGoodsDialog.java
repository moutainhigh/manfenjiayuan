...
//            Looper.loop();
        ...
        if (productEntity.getPriceType().equals(PriceType.WEIGHT)) {
            entity = PosOrderItemService.get().generate(orderBarcode, productEntity, 0 - DataCacheHelper.getInstance().getNetWeight());
        } else {
            if (packFlag == 1) {
                entity = PosOrderItemService.get().generate(orderBarcode, productEntity, 0 - productEntity.getPackageNum());
            } else {
                entity = PosOrderItemService.get().generate(orderBarcode, productEntity, -1D);
            }
        }
    ...
    private void submitOrder(){
        //TODO,新建订单并保存订单明细，标记为挂单状态，然后设置调单。
        //挂起当前订单
//        String orderBarcode = MUtils.getOrderBarCode();
        ...
        //更新订单状态：挂单
        CashierHelper.updateCashierOrder(cashierOrderInfo, PosOrderEntity.ORDER_STATUS_HANGUP);
        cashierOrderInfo.setStatus(PosOrderEntity.ORDER_STATUS_HANGUP);
