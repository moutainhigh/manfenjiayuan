...
import com.mfh.litecashier.ui.fragment.components.IntelligentOrderFragment;
...
public class SimpleDialogActivity extends BaseActivity {
    ...
    public static final int FRAGMENT_TYPE_INTELLIGENT_ORDER         = 0x22;//智能订货
...
//            ZLogger.d("serviceType=" + serviceType);
    ...
    private void initFragments() {
        if (serviceType == FRAGMENT_TYPE_CREATE_PURCHASE_GOODS) {
            ...
            DailySettleFragment fragment;
            Intent intent = this.getIntent();
            if (intent != null) {
                fragment = DailySettleFragment.newInstance(intent.getExtras());
            } else {
                fragment = DailySettleFragment.newInstance(null);
            }
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.fragment_container, fragment)
                    .commit();
        ...
        //采购商品详情
            ...
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.fragment_container, fragment)
                    .commit();
        } else if (serviceType == FRAGMENT_TYPE_INTELLIGENT_ORDER) {
            IntelligentOrderFragment fragment;
            Intent intent = this.getIntent();
            if (intent != null) {
                fragment = IntelligentOrderFragment.newInstance(intent.getExtras());
            } else {
                fragment = IntelligentOrderFragment.newInstance(null);
            }
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.fragment_container, fragment)
                    .commit();
