...
import com.mfh.litecashier.ui.adapter.PurchaseFreshShopcartGoodsAdapter;
 ...
 */
public class PurchaseFreshShopcartFragment extends BaseFragment {
    ...
    private PurchaseFreshShopcartGoodsAdapter goodsAdapter;
    private ItemTouchHelper itemTouchHelper;


    public static PurchaseFreshShopcartFragment newInstance(Bundle args) {
...
//        jsonObject.put("status", OrderStatus.STATUS_CONFIRM);
        ...
        List<PurchaseShopcartOrder> orderList = FreshShopcartHelper.getInstance().getOrderList();
...
//        添加分割线
        ...
        goodsAdapter = new PurchaseFreshShopcartGoodsAdapter(getActivity(), null);
        goodsAdapter.setOnAdapterListener(new PurchaseFreshShopcartGoodsAdapter.OnAdapterListener() {

            @Override
            public void onDataSetChanged(boolean isNeedReloadOrder) {
                if (isNeedReloadOrder) {
                    orderAdapter.setEntityList(FreshShopcartHelper.getInstance().getOrderList());
                } else {
                    //刷新订单列表
    ...
    private void refreshBottomBarInfo() {
        tvOrderQuantity.setText(String.format("订单数: %d",
                FreshShopcartHelper.getInstance().getOrderCount()));
        tvGoodsQunatity.setText(String.format("商品数: %.2f",
                FreshShopcartHelper.getInstance().getTotalCount()));
        tvTotalAmount.setText(Html.fromHtml(String.format("<font color=#000000>商品金额:</font><font color=#FF009B4E>%.2f</font>", FreshShopcartHelper.getInstance().getAmount())));

        if (FreshShopcartHelper.getInstance().getOrderCount() > 0) {
