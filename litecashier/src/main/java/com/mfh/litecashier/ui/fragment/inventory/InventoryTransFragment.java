...
//
    ...
    private void doPayWork(int dialogType, Long orderId, Double amount) {
        ...
        cashierOrderInfo.initSetle(BizType.STOCK, "", String.valueOf(orderId),
                "支付调拨入库单", "", human);
        ...
        payDialog.init(dialogType, cashierOrderInfo.getOrderId(),
...
//        args2.putString(InvTransOrderFragment.EXTRA_KEY_STATUS, String.valueOf(InvTransOrder.STATUS_ON_TRANS));
    ...
    private void loadGoodsList(boolean netFlag, InvTransOrder order) {
        ...
        if (netFlag ) {
            //未签收
            if (OrderStatus.STATUS_ON_TRANS.equals(curOrder.getStatus())){
