...
public class InvRecvOrderFragment extends BaseListFragment<InvSendIoOrder> implements IInvRecvOrderView {
    ...
    protected void createViewInner(View rootView, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (args != null) {
            ...
            payStatus = args.getString(EXTRA_KEY_PAYSTATUS, "");
...
//                ZLogger.d(String.format("%s %d(%d)", (dy > 0 ? "向上滚动" : "向下滚动"), lastVisibleItem, totalItemCount));
        ...
        orderListAdapter = new InvRecvOrderAdapter(CashierApp.getAppContext(), null);
        orderListAdapter.setOnAdapterListener(new InvRecvOrderAdapter.OnAdapterListener() {
            @Override
            public void onItemClick(View view, int position) {
                Bundle args = new Bundle();
                args.putSerializable("order", orderListAdapter.getCurOrder());
                EventBus.getDefault().post(new PurchaseReceiptEvent(PurchaseReceiptEvent.EVENT_ID_RELAOD_ITEM_DATA, args));
            }
     ...
     */
    public void onEventMainThread(InvRecvOrderEvent event) {
        ZLogger.d(String.format("InvRecvOrderFragment: InvRecvOrderEvent(%d)", event.getEventId()));
        if (event.getEventId() == InvRecvOrderEvent.EVENT_ID_RELOAD_DATA) {
            Bundle args = event.getArgs();
            if (args != null) {
                if (payStatus.equals(args.getString(EXTRA_KEY_PAYSTATUS, ""))) {
                    reload();
                }
            }
        } else if (event.getEventId() == InvRecvOrderEvent.EVENT_ID_REMOVE_ITEM) {
            Bundle args = event.getArgs();
            if (args != null) {
...
//            onLoadFinished();
        ...
        invRecvOrderPresenter.loadOrders(mPageInfo, status, payStatus, null);
...
//                    if (mPageInfo.getPageNo() == 1 && orderRecyclerView != null){
