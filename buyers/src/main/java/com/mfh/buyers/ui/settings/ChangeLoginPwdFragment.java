...
public class ChangeLoginPwdFragment extends BaseFragment {
    ...
    private boolean validate(String oldPwd, String newPwd, String confirmNewPwd){
        if(StringUtils.isEmpty(oldPwd)){
            DialogUtil.showHint(R.string.toast_oldpwd_empty);
            return false;
        }
        if(StringUtils.isEmpty(newPwd)){
            DialogUtil.showHint(R.string.toast_newpwd_empty);
            return false;
        }
        if(newPwd.length() < 6){
            DialogUtil.showHint(R.string.toast_newpwd_short);
            return false;
        }
        if(StringUtils.isEmpty(confirmNewPwd)){
            DialogUtil.showHint(R.string.toast_confirmpwd_empty);
            return false;
        }
        if(!newPwd.equals(confirmNewPwd)){
            DialogUtil.showHint(R.string.toast_newpwd_conflict);
            return false;
        }
