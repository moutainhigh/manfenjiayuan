...
//                    InvCheckGoodsService.get().deleteBy(String.format("orderId = '%d'", InvLossOrder.getId()));
//                }
                else if (invLossOrder.getStatus().equals(InvLossOrder.INVLOSS_ORDERSTATUS_FINISHED)) {
                    DialogUtil.showHint("盘点已结束");

                    //删除无效的盘点数据
                    InvCheckGoodsService.get().deleteBy(String.format("orderId = '%d'", invLossOrder.getId()));
                } else if (invLossOrder.getStatus().equals(InvLossOrder.INVLOSS_ORDERSTATUS_CANCELED)) {
