        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    ZLogger.df(String.format("消费结束：%s", response.toString()));
...
//                    isRunningThread = true;
                    ...
                    bankDAO.getCallBack(new BackCall());
                    response = bankDAO.bankall(transCFX, request);
                    ZLogger.df(String.format("撤销订单结束：%s", response.toString()));
                    if (RspCode.CODE_00.equals(response.getRspCode())) {
                        //签到成功，发起消费
...
//        MfhLoginService.get().getCurrentGuId();
        ...
        ZLogger.df("ums.request: " + this.request.toString());
    }
...
//                    {"code":"0","msg":"操作成功!","version":"1","data":""}
                    if (rspData != null) {
                        ...
                        ZLogger.df(String.format("提交支付记录成功:%s", retValue.getValue()));
                    }
