...
public class PurchaseMode {
    ...
    public void loadPurchaseGoods(PageInfo pageInfo, String categoryId, Long otherTenantId,
                                  ...
                                  final OnPageModeListener<FruitScGoodsSku> listener) {
        if (listener != null){
            listener.onProcess();
        }
...
//        }
        ...
        NetCallBack.QueryRsCallBack queryRsCallBack = new NetCallBack.QueryRsCallBack<>(new NetProcessor.QueryRsProcessor<FruitScGoodsSku>(pageInfo) {
            @Override
            public void processQueryResult(RspQueryResult<FruitScGoodsSku> rs) {
                List<FruitScGoodsSku> entityList = new ArrayList<>();
                if (rs != null){
                    for (EntityWrapper<FruitScGoodsSku> wrapper : rs.getRowDatas()) {
        ...
        }, FruitScGoodsSku.class, CashierApp.getAppContext());
